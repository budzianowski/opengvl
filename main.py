...
class BaseModelClient(ABC):
    ...
    @abstractmethod
    def generate_response(self, prompt: str, image_paths: List[str], task_description: str, 
                         example_indices: List[int], selected_indices: List[int], 
...
class JanusClient(BaseModelClient):
    ...
    def generate_response(self, prompt: str, image_paths: List[str], task_description: str,
        ...
        inputs = self.processor.apply_chat_template(
            messages, add_generation_prompt=True, generation_mode="text",
            tokenize=True, padding=True, return_dict=True, return_tensors="pt"
        ).to(self.model.device, dtype=torch.bfloat16)
        
        output = self.model.generate(**inputs, max_new_tokens=400, generation_mode='text', do_sample=False)
