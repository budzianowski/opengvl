...
class InferenceRunner:
    ...
    def load_from_images(
        ...
        completion_predictions = [(i / (len(frames) - 1) * 100) if len(frames) > 1 else 100 
    ...
    def load_from_directory(
            ...
            shuffle: bool = False, seed: int = 42) -> Episode:
    ...
    def run_inference(self, episode: Episode, context_episodes: list[Episode]|None = None) -> dict:
        ...
        try:
            ...
            if extracted_percentages and len(extracted_percentages) == len(episode.frames):
                ...
                if episode.shuffled_frames_indices and len(episode.shuffled_frames_indices) == len(extracted_percentages):
                    try:
    ...
    def plot_results(self, episode: Episode, results: dict, save_path: str|None = None):
        ...
        # Title
        fig.suptitle(f"Inference Results: {episode.instruction}\nVOC Score: {voc_score:.3f}", 
        ...
        if extracted_percentages:
            ...
            # Plot predictions
            ax_comparison.plot(frame_positions, extracted_percentages, 'o-', 
                             label='Model Predictions', linewidth=2, markersize=8, color='red')
            
            # Plot ground truth if available
            if ground_truth_percentages and len(ground_truth_percentages) == len(extracted_percentages):
                ax_comparison.plot(frame_positions, ground_truth_percentages, 's-', 
                                 label='Ground Truth', linewidth=2, markersize=8, color='blue')
            ...
            for i, pred in enumerate(extracted_percentages):
                ax_comparison.annotate(f"{pred}%", (i, pred), textcoords="offset points",
        ...
        if extracted_percentages and episode.shuffled_frames_indices:
            ...
            try:
                ...
                ax_chrono.plot(chrono_positions, chrono_predictions, 'o-', 
                ...
                for pos, pred in zip(chrono_positions, chrono_predictions):
                    ax_chrono.annotate(f"{pred}%", (pos, pred), textcoords="offset points",
                                     xytext=(0, 10), ha="center", fontsize=9)
            except Exception as e:
                logger.warning(f"Could not plot chronological order: {e}")
                ax_chrono.text(0.5, 0.5, "Could not reconstruct chronological order", 
                             ha='center', va='center', transform=ax_chrono.transAxes)
...
def main():
    ...
    data_group.add_argument("--image-dir", type=str,
                           help="Directory containing images")
    data_group.add_argument("--image-files", nargs="+",
                           help="List of image file paths")
    
    # Episode/prompt arguments
    parser.add_argument("--episode-index", type=int, default=0,
                       help="Episode index for dataset loading (default: 0)")
    parser.add_argument("--prompt", type=str,
                       help="Task instruction/prompt (required for image inputs)")
